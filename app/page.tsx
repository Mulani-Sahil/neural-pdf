  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '0',
      role: 'assistant',
      content: 'Hello! I\'m Neural PDF. Upload a PDF and ask me anything about it — or just chat with me directly!','use client';
...
export default function Home() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '0',
      role: 'assistant',
      content: 'Hello! I\'m Neural PDF. Upload a PDF and ask me anything about it — or just chat with me directly!',
      timestamp: new Date(),
    }
  ]);
  ...
  const handleFileUpload = async (file: File) => {
    if (!file.name.toLowerCase().endsWith('.pdf')) {
      addMessage('assistant', '⚠️ Only PDF files are supported.');
      return;
    }
    if (file.size > 10 * 1024 * 1024) {
      addMessage('assistant', '⚠️ File too large. Maximum size is 10MB.');
      return;
    }
    ...
    try {
      ...
      addMessage('assistant', `✅ Got it! I've loaded **${data.filename}**. Ask me anything about it — or anything else!`);
    } catch (err: unknown) {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('/api/ask', {
        ...
        body: JSON.stringify({ question: q, pdfBase64 }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
  ...
  const renderContent = (text: string) => {
    return text
      .split('\n')
      .map((line) => line.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code>$1</code>'))
      .join('<br/>');
  };
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar-label {
          ...
          letter-spacing: 0.1em; text-transform: uppercase; padding: 0 4px; margin-bottom: 8px;
        }
        .pdf-card { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 12px 14px; }
        .pdf-card .pdf-name { color: var(--accent); font-weight: 500; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 4px; }
        .pdf-card .pdf-status { color: var(--text-muted); font-size: 11px; }
        .upload-sidebar-btn {
        ...
        .topbar {
          ...
          align-items: center; padding: 0 24px; gap: 12px; flex-shrink: 0; background: var(--bg);
        }
        .topbar-title { font-size: 14px; font-weight: 500; }
        .topbar-badge {
          font-size: 10px; background: rgba(124,106,247,0.15); color: var(--accent);
          border: 1px solid rgba(124,106,247,0.25); border-radius: 20px; padding: 2px 10px; font-family: var(--mono);
        }
        ...
        .drop-overlay-text { font-size: 18px; font-weight: 600; color: var(--accent); text-align: center; }
      ...
      <div className="layout">
        <aside className="sidebar">
          ...
          <button className="new-chat-btn" onClick={() => {
            ...
            setPdfBase64(null); setFilename(null);
          }}>
        ...
        >
          {dragOver && (
            <div className="drop-overlay">
              <div className="drop-overlay-text">📄<br/>Drop PDF here</div>
            </div>
          )}
